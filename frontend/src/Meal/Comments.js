...
class Comments extends Component {
    ...
    async componentDidMount() {
        let comments = (await axios.get(`http://localhost:8081/comments/` + this.mealId)).data;
        let mealStats = (await axios.get(`http://localhost:8081/mealStats/` + this.mealId)).data;
        this.setState({
            comments,
            mealStats
        });
    ...
    render() {
        ...
        return (
            <div className="comments jumbotron">
                <h3>Comments {mealStats && this.renderStars(Math.round(mealStats.sumStars / mealStats.totalComments))}</h3>
                <hr className="my-4" />
                    ...
                    <div className="container">
                        <form>
                            ...
                            <div className="form-group">
                                <input className="form-control" id="title" placeholder="Title" onChange={e => this.title = e.target.value} required />
                            </div>
                            <div className="form-group">
                                <textarea className="form-control" id="description" rows="3" placeholder="Comment" onChange={e => this.description = e.target.value}></textarea>
                            </div>
                            <button className="btn btn-primary mb-2" onClick={e => this.submit(e)}>Submit</button>
                        </form>
                    </div>
                )}

                <hr className="my-4" />
                {this.state.isAddedNewComment && (<span class="badge badge-success">New comment added !</span>)}
                {comments && comments.map((item, i) => {
                    return <div id={"comment-" + i} className="card comment-card">
                        <div className="card-header comment-header">
                            {this.canDelete(item) && (<button className="float-left btn btn-danger del-comment-btn" onClick={e => this.deleteComment(item, ("comment-" + i))} ><i className="fa fa-trash" aria-hidden="true"></i>
                            </button>)}
                        ...
                        <div className="card-body">
                            <blockquote className="blockquote mb-0">
                                ...
                                <footer className="blockquote-footer">{item.ownerEmail}</footer>
                            </blockquote>
                        </div>
                    </div>
                })}
